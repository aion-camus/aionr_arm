...
impl Classpath {
    ...
    pub fn build(&self, string: &mut String) {
...
impl JavaVM {
    ...
    pub fn new_byte_array_with_data(&self, data: &Vec<u8>) -> Result<Object> {
        ...
        let env: *mut ffi::JNIEnv = ffi::into_raw(self.env);
        unsafe {
            let src: *const u8 = mem::transmute(data.as_ptr());
...
impl<'a> Class<'a> {
    ...
    pub fn call_static(&self, name: &str, args: &[Value], return_type: Type) -> Result<Value> {
        ...
        let result = unsafe {
            ...
            println!("base: {:?}, offset: {:?}, fn_ptr: {:?}", base, offset, fn_ptr);
...
impl<'a> Value<'a> {
    ...
    fn to_jvalue(&self, jvm: &JavaVM) -> ffi::jvalue {
        let data = unsafe {
            match self {
                &Value::Boolean(v) => mem::transmute(v as u32),
                &Value::Byte(v) => mem::transmute(v as u32),
                &Value::Char(v) => mem::transmute(v as u32),
                &Value::Short(v) => mem::transmute(v as u32),
                &Value::Int(v) => mem::transmute(v as u32),
                &Value::Long(v) => mem::transmute(v as u32),
                &Value::Float(v) => mem::transmute(v as u32),
                &Value::Double(v) => mem::transmute(v as u32),
                &Value::Object(ref v) => mem::transmute(v.raw as u32),
